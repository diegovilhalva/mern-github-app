...
const Repo = ({ repo }) => {
	...
	const handleCloneClick = async (repos) => {
		try {
			await navigator.clipboard.writeText(`git clone ${repo.clone_url}`)
			toast.success("Repo URL cloned to clipboard",{position:'top-center'})
			console.log('Repo URL cloned to clipboard')
		} catch (error) {
			toast.error('Clipboard write failed.',{position:'top-center'})
		}
	}
	return (
